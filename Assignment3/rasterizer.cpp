...
void rst::rasterizer::rasterize_triangle(const Triangle &t, const std::array<Eigen::Vector3f, 3> &view_pos) {
    ...
    int x_min = std::min(std::min(t.a().x(), t.b().x()), t.c().x());
    int x_max = std::max(std::max(t.a().x(), t.b().x()), t.c().x());
    int y_min = std::min(std::min(t.a().y(), t.b().y()), t.c().y());
    int y_max = std::max(std::max(t.a().y(), t.b().y()), t.c().y());
        ...
        for (int y = y_min; y <= y_max; ++y) {
            if (insideTriangle(x, y, t.v)) {
                ...
                if (zp < depth_buf[idx]) {
                    ...
                    auto interpolated_texcoords =
                            alpha * t.tex_coords[0] + beta * t.tex_coords[1] + gamma * t.tex_coords[2];
